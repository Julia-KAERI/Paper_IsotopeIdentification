...
The backward cumulative channel sum (BCCS) method was adopted in this study in order to preserve the intrinsic spectral characteristics of plastic scintillation detectors as much as possible while reducing statistical fluctuations. Because plastic scintillator spectra are dominated by broad Compton-related distributions rather than sharp photopeaks, preservation of the overall spectral shape is more important than retaining fine channel-by-channel variations.

During method development, both forward and backward cumulative summation schemes were examined. In the forward cumulative scheme, the cumulative values were calculated as channel 1, channel 1 + channel 2, channel 1 + channel 2 + channel 3, and so on. However, this approach was found to be less effective for radionuclide discrimination because the differences between spectra became less pronounced toward the higher-channel side.

By contrast, in the backward cumulative scheme, each channel value was calculated as the sum from that channel to the final channel. This approach produced more pronounced contrast as the channel number increased, making spectral differences between radionuclides more clearly observable. For this reason, the backward cumulative channel sum (BCCS) was adopted in this study.

The BCCS transformation converts the original spectrum into a monotonically decreasing cumulative representation. This transformation reduces local statistical fluctuations while preserving the global spectral shape of the detector response. As a result, the transformed spectra provide a more stable basis for comparison among radionuclides, particularly under low-count conditions.
...
Let $\mathbf{C} = [C_1,\,C_2,\ldots,\,C_N]$ denote a raw pulse-height spectrum for $N$ channel spectrum where $C_i$ is the count in channel $i$. The Backward Cumulative Channel Sum (BCCS) is defined as
...
After BCCS preprocessing, background compensation was applied to reduce the influence of ambient radiation and system background on the transformed spectra. In practical RPM environments, the measured background may vary depending on container structure, cargo composition, and acquisition conditions. Therefore, background compensation is an essential step before radionuclide decomposition.

Two background compensation techniques were considered in this study. The first was a subtraction-based method, in which the background spectrum was directly subtracted from the measured spectrum. This approach enhances spectral contrast by emphasizing the net source contribution, although it may also amplify statistical fluctuations when the count level is low.

\begin{equation}
S^X_i =  B^X_i - B_i^{\textrm{bkg}}. \label{eq:DS}
\end{equation}

The second was a normalization-based method, in which the measured spectrum was divided by the background spectrum. This method has an important practical advantage in that it reduces dependence on acquisition time. Because the normalized values can be interpreted on a count-rate-equivalent basis, spectra measured under different counting durations can be compared more consistently. This property is particularly useful in real-time inspection systems, where the effective measurement time may vary as a container truck passes the detector.

\begin{equation}
R^X_i = \left(\dfrac{B^X_i}{B^{\textrm{bkg}}_i}\right) -1=\dfrac{S^X_i}{B_i^{\text{bkg}}}.\label{eq:NB}
\end{equation}

In both compensation schemes, the spectral data can also be converted into count-rate form by dividing the measured counts by the acquisition time. In this case, the channel values are no longer restricted to integers and may take fractional values. However, this does not affect the BCCS procedure, because the cumulative summation can be applied equally to integer and non-integer values. Therefore, the proposed method remains applicable even when the measurement time varies. This property is particularly important in practical RPM environments, where container trucks do not always pass the detector at a constant speed.
