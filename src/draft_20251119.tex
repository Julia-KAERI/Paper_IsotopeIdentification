...
\begin{equation}
\argmin_{\alpha_X \ge 0} \left\| \textbf{I}^{\text{M}} - \sum_{X} \alpha_X \textbf{I}^{X}\right\|^2,
\end{equation}
...
\subsection*{Radioisotope identification}

Figure \ref{fig:mixedbccs} ($a$) presents the BCCS spectra processed using the background-subtracted method, whereas \ref{fig:mixedbccs} ($b$) shows the corresponding results obtained through background normalization. Comparing the two reveals that the normalized representation provides a more intuitive indication of the presence of radionuclides. This becomes particularly evident when the overall radiation intensity is relatively high or when the included radionuclides produce Compton peaks at different energy regions. In such cases, the background-normalized spectra highlight these differences more distinctly, allowing mixed-source compositions to be recognized at a glance.
